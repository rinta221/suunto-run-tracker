...
| 1.5 | **slot/result移行** | データモデル再編。既存機能を保ったままスキーマ移行 | **完了**（sessionsはソーク期間後にリネーム退避） |
| 2 | カレンダー版 | 計画TSVインポート・予定行表示・実績紐付け（手動） | ←今ここ（第1スライス：TSVインポート＝**完了**。次：紐付け） |
...
- 既定表示月＝**今日を含む月**（予定が見えることが主目的になるため。当月にslotが無い場合はslotのある最新月へフォールバック）
- **集計の計画除外**：月間距離・月間TRIMP・フェーズまとめ（回数/距離/TRIMP）は status='planned' を数えない
